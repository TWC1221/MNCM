...
module m_iter
  ...
  subroutine iter_xyz_multi(PCG_mode, max_iter, adjoint, fixed_source, subcritical, kinetic)
    use BC_types
    use mesh_types
    implicit none
    ...
    logical, intent(in), optional :: adjoint, fixed_source, subcritical, kinetic
    ...
    real(8), allocatable :: S_f(:), S_f_iter(:), K_eff(:), x(:), chi(:), fission_rate(:)
    ...
    logical :: converged_inner, use_adjoint, use_fixed_source, use_subcritical, use_kinetic
    ...
    if (present(kinetic)) use_kinetic = kinetic

    ! probelm size
    call mesh%init('../mesh_inputs.txt') 
    call read_material_file('../material_inputs_2g.txt', MATs)
    ...
    allocate(S_f(mesh%N), S_f_iter(mesh%N), K_eff(max_iter+1), x(mesh%N), fission_rate(mesh%N))
    ...
    print *, ""
    print*,"Mulitgroup Matrix Assembly Successful"
    ...
    if (use_fixed_source) then
      ...
      do gg = 1,G
        do kk = 1, mesh%nz
          do jj = 1, mesh%ny
            do ii = 1, mesh%nx
              if (ii == 1) then ! if (ii == mesh%nx/2) then ! 
                src_ext(1,ii + (jj-1)*mesh%nx + (kk-1)*mesh%nx*mesh%ny) = (1.00)
              else
                src_ext(gg,ii + (jj-1)*mesh%nx + (kk-1)*mesh%nx*mesh%ny) = 0.0
              end if
            end do
          end do 
        end do
      ...
      do ii = 1, max_iter
        ...
        if (phi_prime(1,mesh%nx/2) > phi(1,mesh%nx/2)) then
          print*,"WARNING: Supercritical - FIXED CONVERGENCE UNPHYSICAL"
          exit
        end if

        converged_inner =.true.
        do gg = 1, G
          if (sum(abs(phi(gg,:) - phi_prime(gg,:))) > 1.0d-4) then
    ...
    print *, ""
    if (use_adjoint) write(*,'(A)') char(27)//"[1;34m"//"===== ADJOINT SOLVE ====="//char(27)//"[0m"
    if (.not. use_adjoint) write(*,'(A)') char(27)//"[1;34m"//"===== PRIMAL (FORWARD) SOLVE ====="//char(27)//"[0m"
    ...
    if (use_kinetic) call kinetic_para(mesh, chi, G)
